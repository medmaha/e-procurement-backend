...
class QuotationRespondRejectView(UpdateAPIView):
    def update(self, request, *args, **kwargs):
        ...
        rfq_response = get_object_or_404(RFQResponse, pk=request.data.get("quote_id"))
        if rfq_response.approved_status != "processing":
            return Response(
        ...
        RFQResponse.objects.filter(pk=rfq_response.pk).update(
